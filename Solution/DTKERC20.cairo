...
#
@storage_var
func account_level_storage(account : felt) -> (level : felt):
end
...
func allowlist_level{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    return account_level_storage.read(account)
end
...
func get_tokens{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
        amount : Uint256):
    let (caller) = get_caller_address()
    let (allowLevel) = allowlist_level(caller)
    # Not even useful, because if level 0 == 0 then it'll try and mint allowLevel(0) * 10 ==> 0
    if allowLevel == 0:
        return (Uint256(0, 0))
    end
    # Making this is bad because then the  user just  has to put  lvl 1000 and he'll  get a LOT of tokens
    # But if we manage the level of users ourself, it should  be fine
    let amount : Uint256 = Uint256(allowLevel * 10, 0)
    ERC20_mint(caller, amount)
    return (amount)
end
...
func request_allowlist{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
        level_granted : felt):
    let (caller) = get_caller_address()
    account_level_storage.write(caller, 1)
    return (1)
end
...
func request_allowlist_level{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (caller) = get_caller_address()
    account_level_storage.write(caller, level_requested)
